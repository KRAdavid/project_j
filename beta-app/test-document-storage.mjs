...
await fileStorage.putBinary({ key: 'LOT-002/COA/binary', contentBase64: binary.toString('base64'), contentSha256: binarySha256 });
...
const s3Storage = new S3DocumentStorage({ endpoint: 'https://objects.example.test', bucket: 'raw-material', accessKeyId: 'ACCESS', secretAccessKey: 'secret', fetchImpl: async (url, options) => {
  ...
  if (options.method === 'PUT') {
    const hash = Buffer.isBuffer(options.body) ? createHash('sha256').update(options.body).digest('hex') : hashDocument(options.body);
    s3Objects.set(key, { content: options.body, hash, authorization: options.headers.authorization });
...
const s3BinarySaved = await s3Storage.putBinary({ key: 'LOT-003/COA/binary', contentBase64: binary.toString('base64'), contentSha256: binarySha256, contentType: 'application/pdf' });
assert.equal(s3BinarySaved.contentSha256, binarySha256);
assert.equal((await s3Storage.getBinary('LOT-003/COA/binary')).contentBase64, binary.toString('base64'));
