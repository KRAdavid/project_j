...
const payload = encode({ sub: 'USER-001', org: 'ORG-001', role: 'BUYER', roles: ['BUYER', 'SUPPLIER'], email_verified: true, iss: 'test-issuer', aud: 'test-audience', exp: Math.floor(Date.now() / 1000) + 3600 });
const signature = createHmac('sha256', process.env.AUTH_JWT_SECRET).update(`${header}.${payload}`).digest('base64url');
const principal = resolvePrincipal({ headers: { authorization: `Bearer ${header}.${payload}.${signature}`, 'x-raw-role': 'OWNER', 'x-active-role': 'SUPPLIER' } }, { environment: 'production' });
assert.deepEqual(principal, { role: 'SUPPLIER', roles: ['BUYER', 'SUPPLIER'], userId: 'USER-001', organizationId: 'ORG-001', environment: 'production' });
const buyerPrincipal = resolvePrincipal({ headers: { authorization: `Bearer ${header}.${payload}.${signature}`, 'x-active-role': 'BUYER' } }, { environment: 'production' });
assert.equal(buyerPrincipal.role, 'BUYER');
assert.throws(() => resolvePrincipal({ headers: { authorization: `Bearer ${header}.${payload}.${signature}`, 'x-active-role': 'OWNER' } }, { environment: 'production' }), (error) => error.code === 'ACTIVE_ROLE_NOT_ALLOWED');
const unverifiedPayload = encode({ sub: 'USER-001', org: 'ORG-001', role: 'BUYER', roles: ['BUYER'], email_verified: false, iss: 'test-issuer', aud: 'test-audience', exp: Math.floor(Date.now() / 1000) + 3600 });
const unverifiedSignature = createHmac('sha256', process.env.AUTH_JWT_SECRET).update(`${header}.${unverifiedPayload}`).digest('base64url');
assert.throws(() => resolvePrincipal({ headers: { authorization: `Bearer ${header}.${unverifiedPayload}.${unverifiedSignature}` } }, { environment: 'production' }), (error) => error.code === 'EMAIL_OWNERSHIP_REQUIRED');
