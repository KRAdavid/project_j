...
첫 화면에서 사업자등록번호와 업무용 이메일로 공통 계정을 등록한 뒤 구매자 또는 공급자를 선택합니다. 같은 계정은 상단 역할 전환으로 양쪽 작업공간을 모두 사용할 수 있습니다. 공급자 화면은 COA 파일과 검증 재고수량 입력이 완료되면 AI 사전검토를 자동 실행하지만, 이는 승인 기능이 아니며 증빙·로트·재고 검증 전에는 매물 공개를 차단합니다.
...
역할별 화면을 직접 열려면 `http://127.0.0.1:4173/?role=buyer`(구매자), `http://127.0.0.1:4173/?role=supplier`(공급자), `http://127.0.0.1:4173/?role=operator`(운영자)를 사용합니다. 별칭 `supplier`는 공급자 화면의 내부 역할 키 `seller`로 안전하게 정규화됩니다.
