...
const result = evaluateTaskSla({
  ...
  tasks: [
    ...
    { id: 'T-BLOCKED', status: 'working', risk: 'critical', createdAt: '2026-09-07T00:00:00.000Z', claimedAt: '2026-09-08T00:00:00.000Z', lastRecheckResult: 'AUTOPILOT_BLOCKED' },
    { id: 'T-EVIDENCE-GAP', status: 'working', risk: 'critical', triggerKey: 'EVIDENCE_GAP', createdAt: '2026-09-07T00:00:00.000Z', claimedAt: '2026-09-08T00:00:00.000Z' },
  ],
});
assert.equal(result.activeCount, 6);
assert.equal(result.metadataCompleteCount, 5);
assert.equal(result.pausedCount, 2);
...
assert.equal(result.items.find((item) => item.taskId === 'T-BLOCKED').pausedForDependency, true);
assert.equal(result.items.find((item) => item.taskId === 'T-BLOCKED').stale, false);
assert.equal(result.items.find((item) => item.taskId === 'T-BLOCKED').action, 'WAIT_FOR_DEPENDENCY');
assert.equal(result.items.find((item) => item.taskId === 'T-EVIDENCE-GAP').pausedForDependency, true);
assert.equal(result.items.find((item) => item.taskId === 'T-EVIDENCE-GAP').stale, false);
